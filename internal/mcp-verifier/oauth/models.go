...
type IOauthFlow interface {
	LoadCachedTokenSource(issuer string, oAuthManager IOAuthManager) (oauth2.TokenSource, error)
	...
	GetOauthTokenSource(meta *AuthServerMetadata, clientID, clientSecret, redirectURI, scope string) (*TokenResponse, error)
}

type IOAuthManager interface {
	// CheckCurrentToken if the token is already saved in the app cache creds file
...
func NewOauthManager() IOAuthManager {
	return &OauthManager{}
}
